...
The redesigned browser experience is intended to make the bottle/ship metaphor more explicit while still visualizing the archive structure and weak-model state transition, not just the controls. The same weak-model simulator can also be explored with sample images or uploaded images, not only through the archive-visualization flow.
...
- exploring the same weak-model simulator with sample images, uploaded images, or the archive-visualization flow
...
In practice, the redesigned page is meant to help you read the BottleShip metaphor and the archive state transition together: sealed oversized archive, keep-set choice, destructive prune, and residual decrypt once the remaining bytes fall at or below the threshold. Whether you explore it through sample images, uploaded images, or the archive-visualization path, it still shows weak-model indicators such as simulated capsule present/deleted state and remaining-bytes-versus-threshold status.
